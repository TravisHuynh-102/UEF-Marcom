        ...
        {dayTrips.slice(0, 3).map((trip) => {
          ...
          return (
            <div
              ...
              className={cn(
                'flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11.5px] font-medium cursor-default truncate transition-all hover:scale-[1.02]',
                cfg.pillBg,
                cfg.pillText,
                !isStart && 'opacity-60'
              )}
            >
              ...
              <span className={cn('ml-auto text-[10.5px] shrink-0', st.color)}>{st.icon}</span>
            </div>
          );
        })}
        {dayTrips.length > 3 && (
          <span className="text-[10.5px] text-[var(--text-muted)] pl-1">+{dayTrips.length - 3} more</span>
        )}
      </div>
    </div>
  );
...
function StaffStatusItem({
  ...
  return (
    <div className="flex items-center gap-3 py-2">
      <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11.5px] font-semibold bg-gradient-to-br from-indigo-500 to-violet-500 text-white shrink-0">
        {getInitials(name)}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11.5px] text-[var(--text-muted)]">{labels[status]}</span>
          {location && status !== 'office' && (
            <span className="text-[11.5px] text-[var(--text-muted)] flex items-center gap-0.5 truncate">
  ...
  return (
    <div className="rounded-xl p-3 border border-[var(--card-border)] bg-[var(--card-bg)] space-y-2.5 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-500/5 transition-all">
      ...
      <div className="flex items-center gap-2.5">
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-[12.5px] font-semibold bg-gradient-to-br from-indigo-500 to-violet-500 text-white">
          {getInitials(trip.employee.name)}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-[var(--text-primary)] truncate">{trip.employee.name}</p>
          <div className="flex items-center gap-1.5">
            <span className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11.5px] font-semibold', cfg.pillBg, cfg.pillText)}>
              {cfg.icon}
              {cfg.label}
            </span>
          </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-[12.5px] text-[var(--text-muted)] line-clamp-2">{trip.purpose}</p>
      </div>
      {/* Actions */}
  ...
  return (
    <div className="flex items-start gap-3 py-2.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={cn('text-[11.5px] font-medium', cfg.pillText)}>{cfg.label}</p>
            ...
            <div className="grid grid-cols-7 border-b border-[var(--card-border)] bg-[var(--bg-tertiary)]">
              {DAYS.map((d) => (
                <div
                  key={d}
                  className={cn(
                    'py-2.5 text-center text-[12.5px] font-semibold uppercase tracking-wider',
                    d === 'Sat' || d === 'Sun'
                      ? 'text-[var(--text-muted)]'
                      : 'text-[var(--text-secondary)]'
                  )}
            ...
            {hasPermission('work_calendar.approve') && pendingTrips.length > 0 && (
              <div className="rounded-2xl border border-violet-500/20 backdrop-blur-xl bg-gradient-to-r from-violet-500/[0.06] to-indigo-500/[0.06] p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="ml-auto inline-flex items-center justify-center w-5 h-5 rounded-md text-[11.5px] font-bold bg-amber-500/20 text-amber-500">
                    {pendingTrips.length}
                  </span>
                </div>
